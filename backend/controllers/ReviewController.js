...
const createReview = async (req, res) => {
    try {
        ...
        if (existing) return res.status(400).json({ message: "Review cho sách này đã tồn tại" });
...
const getAllReviews = async (req, res) => {
    try {
        const reviews = await Review.find().populate("bookId adminId", "title name");
...
const getReviewById = async (req, res) => {
    try {
        const review = await Review.findById(req.params.id).populate("bookId adminId", "title name");
